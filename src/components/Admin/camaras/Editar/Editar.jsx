...
const Editar = ({reFetch, id}) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const {data} = await axios.get(`https://teraflex.cl:9000/admin_enviar_datos?id=${id}`);
                const {camaras} = data;
    ...
    const editar = async () => {
        setLoading(true);
        try {
            const {data} = await axios.post(`https://teraflex.cl:9000/admin_editar_camaras/`, {
                id: id,
                nombre: nombre,
                ubicacion: ubicacion,
                ciudad: selectC
                ...
                <Form layout="vertical" hideRequiredMark form={form}>
                    <Row gutter={16}>
                        <Col span={12}>
                            ...
                            >
                                <Input placeholder="Por favor ingrese nombre de la camara" defaultValue={nombre} onChange={(e) => setNombre(e.target.value)} value={nombre} className='inputText' />
                            </Form.Item>
                        </Col>
                        <Col span={12}>
                            <Form.Item
                            ...
                            >
                                <Input placeholder="Por favor ingrese nombre de la ubicación" defaultValue={ubicacion} onChange={(e) => setUbicacion(e.target.value)} value={ubicacion} className='inputText' />
                            </Form.Item>
                        </Col>
                        <Col span={12}>
                            <Form.Item
