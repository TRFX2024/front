...
const Editar = ({ reFetch, id }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const { data } = await axios.get(`https://teraflex.cl:9000/admin_enviar_datos_ciudad?id=${id}`);
                const { ciudades } = data;
    ...
    const editar = async () => {
        setLoading(true);
        try {
            const { data } = await axios.post(`https://teraflex.cl:9000/admin_editar_ciudad/`, {
                id: id,
                nombre: nombre,
            });
            messageApi.open({
                ...
                <Form layout="vertical" hideRequiredMark form={form}>
                    <Row gutter={16}>
                        <Col span={12}>
                            ...
                            >
                                <Input placeholder="Por favor ingrese nombre de la ciudad" defaultValue={nombre} onChange={(e) => setNombre(e.target.value)} value={nombre} className='inputText' />
                            </Form.Item>
                        </Col>
                    </Row>
                </Form>
